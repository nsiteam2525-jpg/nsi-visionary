...
function EditDialog({ dream, onClose }: { dream: Dream; onClose: () => void }) {
  ...
  const [startedDate, setStartedDate] = useState<string>(toDateInput(dream.created_at));
  const [achievedDate, setAchievedDate] = useState<string>(toDateInput(dream.achieved_at ?? dream.updated_at));
    ...
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4" onClick={onClose}>
      <div className="glass-strong p-6 max-w-lg w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          ...
          <label className="block"><div className="text-[11px] uppercase tracking-widest text-gold/80 mb-1">Started Date</div>
            <input type="date" className={inputCls} value={startedDate} onChange={(e) => setStartedDate(e.target.value)} /></label>
          <label className="block"><div className="text-[11px] uppercase tracking-widest text-gold/80 mb-1">Achieved Date</div>
            <input type="date" className={inputCls} value={achievedDate} onChange={(e) => setAchievedDate(e.target.value)} /></label>
        ...
        <div className="mt-5 flex gap-2">
          <button onClick={() => {
            ...
            const patch: any = { ...d, id: dream.id };
            if (startedDate) patch.created_at = new Date(startedDate).toISOString();
            if (achievedDate) patch.achieved_at = new Date(achievedDate).toISOString();
            save.mutate(patch, { onSuccess: () => { toast.success("Updated"); onClose(); } });
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-[88rem] px-4 sm:px-8 py-10">
        ...
        <div className="mt-6 grid sm:grid-cols-3 gap-4">
          <div className="glass-strong p-5"><div className="text-[11px] uppercase tracking-widest text-muted-foreground">Dreams Achieved</div><div className="mt-2 font-display text-3xl text-gradient-gold">{achieved.length}</div></div>
        ...
        {isLoading ? (
          ...
          <div className="glass p-12 mt-8 text-center">
            ...
            <p className="text-muted-foreground">No dreams achieved yet. Click the 🏆 Trophy icon on a dream when you finish it and it will land here.</p>
            <Link to="/dreams" className="btn-gold rounded-full px-6 py-3 mt-6 inline-block font-semibold">Go to Dreams</Link>
          </div>
        ) : (
          <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {achieved.map((d) => {
              const days = daysBetween(d.created_at, d.achieved_at ?? d.updated_at);
              ...
              return (
                <motion.div key={d.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="glass-strong p-5 relative overflow-hidden group">
                  <div className="absolute top-3 right-3 flex items-center gap-1 z-10">
                    ...
                    <button title="Move back to active dreams" onClick={() => { if (confirm(`Move "${d.name}" back to active dreams?`)) save.mutate({ id: d.id, name: d.name, is_achieved: false, achieved_at: null } as any, { onSuccess: () => toast.success("Moved back to Dreams") }); }} className="p-1.5 rounded-md text-muted-foreground hover:text-gold hover:bg-white/5"><Undo2 size={15} /></button>
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
                    ...
                    <div className="glass p-2"><div className="text-muted-foreground flex items-center gap-1"><Trophy size={11}/> Achieved</div><div className="mt-1">{(d.achieved_at ?? d.updated_at) ? new Date((d.achieved_at ?? d.updated_at)!).toLocaleDateString("en-IN") : "—"}</div></div>
                  </div>
                  {days && (
